...
with app.app_context():
    try:
        ...
        # কন্ট্রোলার অ্যাকাউন্ট ক্রিয়েট/আপডেট (srs10. / 12qw3e)
...
def login():
    ...
    if user and check_pw(password, user.password_hash):
        if user.role == 'admin' and not user.is_approved:
            flash("আপনার অ্যাডমিন অ্যাকাউন্টটি এখনও অনুমোদিত হয়নি! কন্ট্রোলারের অনুমোদনের অপেক্ষা করুন।", "warning")
...
def controller_dashboard(username):
    ...
    if not current_user_id or session.get('role') != 'controller':
        flash("প্রবেশের অনুমতি নেই! সঠিক অ্যাকাউন্ট দিয়ে লগইন করুন।", "danger")
        return redirect(url_for('home'))
    ...
    approved_admins = User.query.filter_by(role='admin', is_approved=True).all()
...
def approve_admin(admin_id):
    ...
    if admin_user:
        ...
        db.session.commit()
        flash(f"{admin_user.username} - অ্যাকাউন্টটি সফলভাবে অনুমোদন করা হয়েছে!", "success")
    
    current_user = User.query.get(session.get('user_id'))
    return redirect(url_for('controller_dashboard', username=current_user.username))
...
def admin_signup():
    if request.method == 'POST':
        ...
        if User.query.filter((User.username == data['username']) | (User.mobile == data['mobile'])).first():
            flash("এই ইউজারনাম বা মোবাইল নম্বরটি দিয়ে আগেই অ্যাকাউন্ট খোলা হয়েছে!", "danger")
...
def admin_dashboard(username):
    current_user_id = session.get('user_id')
    if not current_user_id or session.get('role') != 'admin':
        flash("প্রবেশের অনুমতি নেই! সঠিক অ্যাডমিন অ্যাকাউন্ট দিয়ে লগইন করুন।", "danger")
        return redirect(url_for('home'))
        
    user = User.query.get(current_user_id)
    if not user or user.username != username or not user.is_approved:
        flash("আপনার অ্যাকাউন্টটি সক্রিয় নয়।", "danger")
        return redirect(url_for('home'))
