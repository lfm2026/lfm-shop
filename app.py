...
# 🔑 আপনার সেট করা সিক্রেট পাসওয়ার্ড
...
def update_stock():
    ...
    if not text_input:
        return jsonify({'status': 'error', 'message': 'টেক্সট ইনপুট দেওয়া হয়নি!'}), 400

    if not groq_client:
        return jsonify({'status': 'error', 'message': 'GROQ API key পাওয়া যায়নি!'}), 400
...
def verify_address():
    ...
    try:
        prompt = f"Verify and format this Bangladeshi shipping address clearly (Village/Area, Upazila, District): {raw_address}"
...
def facebook_webhook():
    if request.method == 'GET':
        ...
        data = request.json
        # ফেসবুক/ইনস্টাগ্রাম মেসেজ হ্যান্ডলিং লজিক
